...
[time_series1,y_train] = time_reshape(win_length,x,fs,y);
...
time_input = (time_input' * pca_coeT2(:,1:60))';
...
save('norm_time_all.mat','norm_time1');
...
% 
% 
% fprintf('The accuracy of freq domain LD through 5-fold cross-validation is %.2f %% \n',...
%     (1-model_LD_f.HyperparameterOptimizationResults.MinObjective))
% 
% fprintf('The accuracy of SVM through 5-fold cross-validation is %.2f %% \n',...
%     (1-model_svm_f.HyperparameterOptimizationResults.MinObjective))
...
%y_predict(:,3) = predict(ens,norm_time1(start_num+1:end,:));
% y_predict(:,3) = predict(model_LD_f,norm_gamma1(start_num+1:end,:));
% y_predict(:,4) = predict(model_svm_f,norm_gamma1(start_num+1:end,:));
...
acc1 = 1 - sum(abs(y_predict(:,3)-y_test))/length(y_test)
